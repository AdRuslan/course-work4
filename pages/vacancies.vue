<template>
    <div>
      <v-btn v-if="!isWithXp" v-on:click="show_xp" >Отобразить только вакансии с требуемым опытом(фильтрация)</v-btn>
      <v-btn v-if="isWithXp" v-on:click="show" >Отобразить все вакансии</v-btn>
      <div class="pages">
...
export default {
  ...
  components: {Order},
  async fetch({store}) {
    if (store.getters['vacs/vacancies'].length === 0) {
      await store.dispatch('vacs/fetch')
    }
  },
  computed: {
    vacancies() {
        return this.$store.getters['vacs/vacancies'];
    }
  },
  methods: {
    async show() {
    ...
    async sortNames() {
      ...
      const vacancies = this.$store.getters['vacs/vacancies'];
      await this.$store.dispatch('vacs/sortByNames', vacancies);
    },
    async drop() {
      this.isWithXp = false
      await this.$store.dispatch('vacs/fetch')
